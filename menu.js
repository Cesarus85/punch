...
function lockInitialPose() {
  ...
  setLasersVisible(true);   // sichtbare Raycasts im Menü
...
hud.plane.visible = false;  // kein Pausesymbol, HUD erst ingame sichtbar

// ---------- Fäuste ----------
const fistsMgr = new FistsManager(renderer, scene, {
  showControllerModels: true,        // echte Controller wenn verfügbar
  sphereVisRadius: 0.03              // sonst kleine gelbe Kugeln (3 cm)
});
...
const DIFFICULTY_DRIFT = { 'Anfänger': 1.00, 'Aufsteiger': 0.70, 'Profi': 0.25 }; // Anteil „gerade“
...
function beginCountdown() {
  ...
  // NICHT resetten – Spiel startet frisch nach Countdown, vorher kein Ingame
...
// ---------- Controller Rays (Laser) – nur im Menü sichtbar ----------
const raycaster = new THREE.Raycaster();
const controllers = [renderer.xr.getController(0), renderer.xr.getController(1)];
const lasers = [];

function makeLaser() {
  // sichtbarer Laser-Strahl (Mesh), gut erkennbar, rendert „on top“
  const len = 2.5;
  ...
  mesh.rotation.x = Math.PI / 2; // Zylinder zeigt entlang -Z nach vorne
...
for (const c of controllers) {
  ...
  c.addEventListener('selectstart', () => {
    ...
    else if (action.action === 'resume') { // nur im Ingame-Menü
      menu.setVisible(false);
      setLasersVisible(false);
      game.menuActive = false;
      game.running = true; // weiterlaufen
...
// ---------- A/X Face-Button: Menü ein/aus & Pause/Resume ----------
function isRisingEdgeAX(gp, storeKey, store){
  if (!gp || !gp.buttons) return false;
  // exakt NUR Face-Buttons A/X (typisch Index 3 oder 4)
  const pressed = !!(gp.buttons[3]?.pressed) || !!(gp.buttons[4]?.pressed);
  const prev = !!store[storeKey]; store[storeKey] = pressed;
  return pressed && !prev;
}
function toggleMenuViaFace(){
  if (!game.menuActive) {
    // Spiel -> pausieren & Menü (ingame)
    game.running = false;
    ...
    // Menü -> fortsetzen (nur wenn ingame)
    if (menu.group.visible) {
      menu.setVisible(false);
      setLasersVisible(false);
      game.menuActive = false;
      game.running = true;
...
function loop(){
  ...
  // A/X auf beiden Controllern abfragen (keine Thumbsticks!)
  const session = renderer.xr.getSession?.();
  if (session){
    if (!loop._btnPrev) loop._btnPrev = {};
    for (const src of session.inputSources) {
      const gp = src.gamepad; if (!gp) continue;
      if (isRisingEdgeAX(gp, `${src.handedness}:AX`, loop._btnPrev)) toggleMenuViaFace();
    }
  }
  ...
  // Menü-Hover: Laser im Menü ständig aktualisieren
  if (game.menuActive) {
    for (const c of controllers) {
      const origin = new THREE.Vector3(), dir = new THREE.Vector3(0,0,-1);
      c.getWorldPosition(origin); dir.applyQuaternion(c.quaternion).normalize();
      ...
      // Laser-Länge/Orientierung: liegt als Child bereits korrekt – optional könnte man Länge dynamisch machen
    }
  }
...
renderer.xr.addEventListener('sessionend', () => {
  ...
  balls.length=0; hazards.length=0;
  menu.setVisible(false); setLasersVisible(false); game.menuActive=false; hud.plane.visible=false;
});
start();
